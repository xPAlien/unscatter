...
export const OutputSection: React.FC<OutputSectionProps> = ({ result }) => {
  ...
  return (
    <div className="space-y-10">
      {nextActionTask && (
        <div>
          <h2 className="text-sm font-bold uppercase text-blue-600 tracking-wider mb-3">Next Actionable Step</h2>
          <TaskCard task={nextActionTask} isNextAction={true} />
        </div>
      )}
      
      <div>
        <h2 className="text-sm font-bold uppercase text-gray-500 tracking-wider mb-4 border-b pb-2">Full Plan</h2>
        <div className="space-y-6">
          {Object.entries(tasksByCluster).map(([cluster, clusterTasks]) => (
            <div key={cluster}>
              <h3 className="text-lg font-semibold text-gray-800 mb-3">{cluster}</h3>
